...
export class TickTonesSettingsTab extends PluginSettingTab {
  ...
  private createNoSoundsFoundSection(containerEl: HTMLElement) {
    ...
    new Setting(containerEl)
      ...
      .setDesc("You're just some small steps away from unlocking the plugin's full potential.")
    ...
    new Setting(containerEl)
      ...
      .setDesc("Reload the plugin or use the Reload button below once you've added sounds.")
    new Setting(containerEl)
      ...
      if (this.plugin.settings.useRandomTickSound) {
        ...
        if (this.plugin.settings.tickSounds.length > 0) {
          ...
          new Setting(containerEl)
            ...
            .setDesc('Click on sounds above to add them to the random selection.')
        }
      } else {
        new Setting(containerEl)
          .setName('Tick sound')
  ...
  private createUntickSoundsSection(containerEl: HTMLElement, sounds: string[]) {
    new Setting(containerEl).setName('Untick sound').setHeading()

    new Setting(containerEl)
      .setName('Untick sound enabled')
      .setDesc('Toggle if a sound should be played when a checkbox is unticked.')
      .addToggle((toggle) => {
